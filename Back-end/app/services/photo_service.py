...
CARD_WIDTH = 800
CARD_HEIGHT = 500
PASSPORT_WIDTH = 170  # Ajusté pour l'ergonomie
PASSPORT_HEIGHT = 290 # Ajusté pour l'ergonomie
...
def generate_student_card(etudiant_data, photo_path, qr_code_cle):
    try:
        ...
        try:
            ...
            font_subtitle = ImageFont.truetype(fontP, 14)
        ...
        photo_x, photo_y = 70, 170
        draw.rounded_rectangle([photo_x, photo_y , photo_x + PASSPORT_WIDTH , photo_y + PASSPORT_HEIGHT - 63], radius=20, fill=UCB_BLUE)
        ...
        try:
            with Image.open(full_photo_path) as img:
                ...
                draw.rounded_rectangle([68, photo_y-2, 72+p_size[0], photo_y+p_size[1]+2], radius=17, outline=UCB_GOLD, width=3)
        except:
        ...
        draw.line([50, 420, CARD_WIDTH - 200, 420], fill=UCB_GOLD, width=1)
        
        # Année académique et le tag unique #Plmd_Nz
        draw.text((250, 440), "Année Académique: 2025-2026", font=font_tag, fill=TEXT_WHITE) 
        draw.text((290, 460), "@by Plmd_Nz", font=font_tag, fill=UCB_GOLD)
